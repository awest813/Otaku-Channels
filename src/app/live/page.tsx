  ...
  return (
    <div className='mx-auto max-w-screen-xl px-4 py-8'>
      ...
      <p className='mb-8 max-w-2xl text-slate-400'>
        Free 24/7 anime streaming channels. Tune in to live and scheduled
        programming from official sources.
      </p>
      ...
      <div className='mt-10'>
        ...
        <div className='overflow-hidden rounded-xl border border-slate-800 bg-slate-900'>
          ...
          {channels.map((ch) => (
            ...
            >
              <div className='px-3 py-3 font-mono text-cyan-300'>
                {ch.channelNumber}
              </div>
              <div className='px-3 py-3 text-white'>{ch.nowPlaying}</div>
              <div className='px-3 py-3 text-slate-400'>{ch.nextUp ?? '—'}</div>
              <div className='px-3 py-3 text-right text-xs text-slate-500'>
                {ch.sourceName}
              </div>
            </div>
          ))}
