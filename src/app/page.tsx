...
export default function HomePage() {
  ...
  const dubbed = mockSeries.filter((s) => s.language === 'dub' || s.language === 'both');
  ...
  return (
    <>
      ...
      <div className='mx-auto max-w-screen-xl space-y-10 px-4 py-10'>
        <MediaRail title='Trending Free Anime' items={trending} seeAllHref='/browse' />
        <MediaRail title='Official YouTube Anime' items={youtube} seeAllHref='/browse?source=youtube' />
        <MediaRail title='Retro Anime' items={retro} seeAllHref='/browse?source=retro' />
        <MediaRail title='Dubbed Picks' items={dubbed.slice(0, 6)} seeAllHref='/browse?language=dub' />
        <MediaRail title='Movies' items={movies as Array<import('@/types').AnimeSeries | import('@/types').Movie>} seeAllHref='/browse?type=movie' />
