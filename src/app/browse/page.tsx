...
const sources = ['All', 'YouTube Official', 'Retro Channel', 'FreeStream Anime'];
...
export default function BrowsePage() {
  ...
  const filtered = (allContent as Array<AnimeSeries | Movie>).filter((item) => {
    ...
    if (lang !== 'All') {
      const l = 'language' in item ? item.language : 'sub';
      if (l !== lang && !(lang === 'dub' && l === 'both') && !(lang === 'sub' && l === 'both')) return false;
    }
    return true;
  });
  ...
  return (
    <div className='mx-auto max-w-screen-xl px-4 py-8'>
      <h1 className='mb-6 text-2xl font-bold text-white md:text-3xl'>Browse All Anime</h1>
