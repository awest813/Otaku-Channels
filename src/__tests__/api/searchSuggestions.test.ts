...
describe('GET /api/search/suggestions', () => {
  ...
  it('returns suggestions matching a known title in mock data', async () => {
    ...
    const titles = body.data.map((s: { title: string }) => s.title.toLowerCase());
  ...
  it('each suggestion has required fields', async () => {
    ...
    const body = await res.json();
    body.data.forEach((s: { slug: string; title: string; posterUrl: string | null }) => {
      expect(typeof s.slug).toBe('string');
      expect(typeof s.title).toBe('string');
      // posterUrl can be string or null
      expect(s.posterUrl === null || typeof s.posterUrl === 'string').toBe(true);
    });
  });
});
