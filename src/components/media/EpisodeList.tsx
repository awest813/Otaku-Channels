...
interface Props {
  episodes: Episode[];
  /**
   * Slug of the parent series.  When provided the internal watch route gets
   * `?series=<slug>&ep=<episodeNumber>` appended so the watch page can look
   * up the next episode for autoplay.
   */
  seriesSlug?: string;
}

export default function EpisodeList({ episodes, seriesSlug }: Props) {
    ...
    <div className='space-y-2'>
      {episodes.map((ep, index) => {
        ...
        return (
          <Link
            key={ep.id}
            href={ep.isEmbeddable ? internalHref : ep.watchUrl}
          ...
          >
            ...
            <div className='min-w-0 flex-1 pt-0.5'>
              ...
              <p className='mb-0.5 text-xs font-medium text-slate-500'>
                ...
                {ep.language && ep.language !== 'both' && (
                  <span className='ml-2 rounded bg-slate-800 px-1.5 py-0.5 text-[10px] uppercase'>
                    {ep.language}
                  </span>
                )}
              </p>
